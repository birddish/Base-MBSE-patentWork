...
> 本文件为 `mbse-patentability-analysis` 全 Mode 通用的 R/F/S/B/E 五层建模框架。所有 Mode 均使用此五层模型，仅在建模深度、追溯方向和证据严格度上分化。具体差异参见各 mode 工作流文件（`references/modes/mode-*.md`）。
...
本 skill 在阶段一强制输出 Mermaid 可视化图表，以辅助技术方案的直观理解。以下为各视角图表的建模规范。
