...
本文件定义 `mbse-patentability-analysis` skill disclosure 模式阶段三的检索要素提取规则、JSON 格式规范，以及与后续公开资料初筛和正式专利检索的衔接说明。详细工作流参见 `references/modes/mode-disclosure.md`。
...
```json
{
  "innovation_id": 1,                          // 整数，创新点编号
  "innovation_name": "字符串",                 // 创新点名称
  "technical_problem": "字符串",               // 现有技术缺陷（从需求视角R提取）
  "technical_solution": "字符串",              // 本方案技术路径（从F+S+B提取）
  "technical_effect": "字符串",                // 技术效果
  "evidence_level": "disclosed | inferred | question",
  "search_priority": "high | medium | low",
  "search_elements": {                        // 检索要素
    "core_keywords_zh": ["字符串数组"],        // 核心中文关键词（3-6个）
    "core_keywords_en": ["字符串数组"],        // 核心英文关键词（3-6个）
    "extended_keywords": ["字符串数组"],       // 扩展关键词（同义词、上位、下位，6-12个）
    "ipc_candidates": ["字符串数组"],          // IPC候选分类号（1-3个）
    "ipc_confidence": "字符串",                // 置信度：高/中/低
    "ipc_inference": "字符串",                 // 推断依据说明
    "noise_exclusion": ["字符串数组"],         // 排除词/噪声过滤（2-5个）
    "key_combinations": ["字符串数组"]         // 关键词组合检索式（2-4个）
  },
  "novelty_indicators": {                      // 新颖性假设/检索后指标
    "blank_spot": "字符串",                    // 未检索时写“待检索验证”；实际检索后才可写专利库空白描述
    "differentiation": "字符串"                // 与已知现有技术或背景技术的核心差异；未检索时不得写成最终结论
  },
  "related_patents_from_search": ["字符串数组"] // 已知相关专利号（如有）
}
```
...
```
mbse-patentability-analysis skill
├── 阶段一：四视角系统建模 + 可视化图表
├── 阶段二：创新点识别 + 技术方案图解
└── 阶段三：检索要素结构化输出
         │
...
└── 输出：04_google_patents_search_log.md
         │
         ▼
patent-search-query-builder skill
...
| mbse-patentability-analysis 输出 | patent-search-query-builder 输入 | 说明 |
|----------------------------|--------------------------------|------|
...
当用户基于 `mbse-patentability-analysis` 完成分析后，如需执行专利检索，建议以下话术：

> "基于以上MBSE分析，已为每个创新点提取了结构化检索要素。是否需要基于这些检索要素，调用专利检索式构建 skill 生成完整检索方案（含检索式、策略建议、执行顺序）？"
