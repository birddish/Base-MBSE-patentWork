...
def validate_lightweight_runtime(run_dir: Path, errors: list[str]) -> None:
    """Validate quick/standard search handoffs without invoking V2 orchestration."""
    context_path = run_dir / "search_context.json"
    handoff_path = run_dir / "search_handoff.json"
    if not context_path.is_file() or not handoff_path.is_file():
        errors.append("lightweight run requires search_context.json and search_handoff.json")
        return
    context = load_json(context_path, errors)
    handoff = load_json(handoff_path, errors)
    profile = context.get("delivery_profile")
    if context.get("schema_version") != "search-context/lightweight-1.0" or profile not in {"quick", "standard"}:
        errors.append("lightweight search_context requires schema_version search-context/lightweight-1.0 and profile quick/standard")
        return
    if handoff.get("schema_version") != "search-handoff/lightweight-1.0" or handoff.get("delivery_profile") != profile:
        errors.append("lightweight search_handoff schema/profile mismatch")
    for field in ["case_id", "mode", "mode_payload", "required_languages", "required_jurisdictions", "feature_set_id", "input_scope"]:
        if context.get(field) in (None, "", []):
            errors.append(f"lightweight search_context missing {field}")
    validate_search_mode_context(context, "lightweight search_context", errors)
    if handoff.get("case_id") != context.get("case_id") or handoff.get("mode") != context.get("mode"):
        errors.append("lightweight context/handoff case or mode mismatch")
    queries = handoff.get("queries")
    if not isinstance(queries, list) or not queries:
        errors.append("lightweight search_handoff requires at least one query")
    else:
        for index, query in enumerate(queries):
            if not isinstance(query, dict) or not query.get("query_text") or not query.get("database") or parse_time(query.get("executed_at")) is None:
                errors.append(f"lightweight query[{index}] is incomplete")
    if not isinstance(handoff.get("raw_result_files"), list) or not handoff.get("raw_result_files"):
        errors.append("lightweight search_handoff requires raw_result_files")
    distribution = handoff.get("jurisdiction_distribution")
    if not isinstance(distribution, dict) or set(distribution) != {"CN", "US", "JP", "KR", "EP", "WO"} or not all(isinstance(value, int) and value >= 0 for value in distribution.values()):
        errors.append("lightweight search_handoff requires six-jurisdiction distribution")
    for field in ["documents", "evidence_links", "risk_register"]:
        if not isinstance(handoff.get(field), list):
            errors.append(f"lightweight search_handoff {field} must be an array")
    forbidden = {"verified_findings", "writeback_objects", "attorney_confirmation_id", "execution_record_id", "execution_record_hash", "query_run_ids", "query_run_refs"}
    for field in forbidden:
        if handoff.get(field) not in (None, [], ""):
            errors.append(f"lightweight search_handoff must not contain formal field {field}")
    for document in handoff.get("documents") or []:
        if isinstance(document, dict) and document.get("classification") in {"X", "Y", "A"}:
            errors.append("lightweight search_handoff must not classify documents as X/Y/A")
        if profile == "standard" and context.get("mode") == "fto" and isinstance(document, dict) and document.get("risk_candidate") and not document.get("legal_status"):
            errors.append("standard FTO risk candidate lacks legal_status")


def lightweight_profile(run_dir: Path, errors: list[str]) -> str | None:
    context_path = run_dir / "search_context.json"
    if not context_path.is_file():
        return None
    context = load_json(context_path, errors)
    profile = context.get("delivery_profile")
    return profile if profile in {"quick", "standard"} else None
...
def validate_query_runs(run_dir: Path, context: dict[str, Any], handoff: dict[str, Any],
    ...
    for run_id in run_ids:
        ...
        if benchmark_status not in {"not_applicable", "passed", "pending", "failed"}:
            errors.append(f"{item_label}: benchmark_status is invalid")
        elif benchmark_status == "passed" and not expected_benchmarks.issubset(hits):
            errors.append(f"{item_label}: passed benchmark_status conflicts with missing benchmark hits")
        elif expected_benchmarks and not expected_benchmarks.issubset(hits) and benchmark_status not in {"pending", "failed"}:
            errors.append(f"{item_label}: missing benchmark hits must be recorded as pending or failed")
        if benchmark_status in {"pending", "failed"} and not value.get("follow_up_reason"):
            errors.append(f"{item_label}: pending/failed benchmark requires follow_up_reason")
        if value.get("run_status") not in {"executed", "conditional_pass"}:
            errors.append(f"{item_label}: run_status is invalid")
        if value.get("truncation_status") not in {"unknown", "not_observed", "suspected", "confirmed"}:
            errors.append(f"{item_label}: truncation_status is invalid")
...
def main() -> int:
    ...
    if args.run_dir is not None:
        ...
        profile = lightweight_profile(run_dir, errors)
        if profile is not None:
            errors.append(f"{profile} delivery profile is not a formal V2 run; use validate_lightweight_search.py")
        else:
            trust_store = None
