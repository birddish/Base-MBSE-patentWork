#!/usr/bin/env python3
"""Create a non-overwriting v2.1-oa copy of a legacy/v2.0 OA evidence pack."""
...
from oa_v21_contract import V21_TOP_LEVEL, upgrade_to_v21
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("input", type=Path, help="legacy patent_evidence_pack.json")
    parser.add_argument("output", type=Path, help="new v2.1-oa JSON path; must differ from input")
...
def normalize_v1(data: dict[str, Any], run_dir: Path | None) -> dict[str, Any]:
    ...
    return {
        ...
        + [{"version": "2.0-oa-regression", "change": "从旧包生成非覆盖的 OA schema 回归副本。", "status": "draft"}],
...
def normalize_case3(data: dict[str, Any], run_dir: Path | None) -> dict[str, Any]:
    ...
    inventive = [
        {
            ...
            "technical_effects": [],
            "objective_technical_problem": "旧包未以 v2.0-oa 结构化记录，见原追溯链报告。",
    ...
    return {
        ...
        "version_history": [
            {"version": "2.0-oa-regression", "change": "从异构旧包生成非覆盖的 OA schema 回归副本。", "status": "draft"}
        ],
...
def main() -> int:
    ...
    if data.get("schema_version") in {"2.0-oa", "2.1-oa"}:
        base = copy.deepcopy(data)
    elif isinstance(data.get("case"), dict):
        base = normalize_v1(data, run_dir)
    else:
        base = normalize_case3(data, run_dir)
    normalized = upgrade_to_v21(base)
    missing = [key for key in (*REQUIRED_TOP_LEVEL, *V21_TOP_LEVEL) if key not in normalized]
    if missing:
