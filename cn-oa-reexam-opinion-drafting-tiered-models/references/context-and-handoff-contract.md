...
本合同采用`multi-model-orchestration/2.0`、`multi-model-task/2.0`、`multi-model-handoff/2.0`、`multi-model-routing/2.0`和`multi-model-invalidation/2.0`。所有1.x运行目录一律拒绝；须新建V2目录，不提供迁移器。案件文件是唯一事实源，聊天历史和模型摘要不能恢复正式状态。
