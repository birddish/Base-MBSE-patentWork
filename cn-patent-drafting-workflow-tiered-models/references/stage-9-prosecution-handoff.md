...
本阶段在提交申请后、收到审查意见通知书或驳回决定时启动。它不替代 `cn-oa-reexam-opinion-drafting-tiered-models` 的五阶段答复流程，而是提供统一输入、版本控制和回写规则。交接必须读取`cross-skill-adapters.md`，向OA技能发送机器可读`oa_context.json`，并只接收与该输入快照匹配的`oa_handoff.json`。
...
生成 `prosecution_handoff.md`和`oa_context.json`，至少列出：案件基准日、当前 `CLM/CF/EV/DOC/NB/IS` ID、提交版本及哈希、输入快照、审查员主张、引用文献、已核验事实、待核验事项、可修改特征及其原始支持位置。将二者与原材料交给 `cn-oa-reexam-opinion-drafting-tiered-models`。OA完成复核和重算后返回`oa_handoff.json`；只有`recalculation_gate_passed=true`且输入哈希匹配的返回包才可进入活动版本候选，仍须代理师确认。
